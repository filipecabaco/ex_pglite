defmodule PgliteIntegrationTest do
  ...
  test "executes simple queries", %{temp_dir: temp_dir} do
    {:ok, {_manager, conn}} = start_pglite(data_dir: temp_dir)
  ...
  test "handles connection failures", %{temp_dir: temp_dir} do
    ...
    {:ok, {manager, conn}} = start_pglite(data_dir: temp_dir)

    assert {:ok, _result} = Postgrex.query(conn, "SELECT 1", [])
    refute_receive {:EXIT, ^manager, {:pglite_exit, _status}}
  ...
  test "can run multiple instances simultaneously with different data" do
    {:ok, {_manager1, conn1}} = start_pglite(data_dir: "tmp/pglite_integration_multi_1")
    {:ok, {_manager2, conn2}} = start_pglite(data_dir: "tmp/pglite_integration_multi_2")
    {:ok, {_manager3, conn3}} = start_pglite(data_dir: "tmp/pglite_integration_multi_3")
  ...
  test "concurrent operations on different instances don't interfere" do
    ...
    {:ok, {_manager1, conn1}} = start_pglite(data_dir: "tmp/pglite_integration_concurrent_1")
    {:ok, {_manager2, conn2}} = start_pglite(data_dir: "tmp/pglite_integration_concurrent_2")
  ...
  defp start_pglite(opts) do
    {:ok, {manager, _conn}} = result = Pglite.start_link(opts)
